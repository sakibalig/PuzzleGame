...
document.getElementById('new-game').onclick=function(){
    ...
    for(let i=0;i<3;i++){
        for(let j=0;j<3;j++){
            swap('dig'+Math.floor(Math.random()*9+1),'dig'+Math.floor(Math.random()*9+1));
        }
    }
    /*for(let i=1;i<9;i++){
        if(document.getElementById('dig'+i).innerHTML<'9'){
            document.getElementById('dig'+i).style.backgroundColor='rgb(162, 255, 0)';
        }
        if(document.getElementById('dig'+i).innerHTML==none){
